...
class TwoStageDetector(BaseDetector, RPNTestMixin, BBoxTestMixin,
    ...
    def init_weights(self, pretrained=None):
        ...
        if self.with_neck:
            if isinstance(self.neck, nn.Sequential) or isinstance(self.neck, nn.ModuleList):
                for m in self.neck:
                    m.init_weights()
            else:
    ...
    def extract_feat(self, img):
        ...
        if isinstance(self.backbone, nn.ModuleList):
            ...
            for i in range(len(self.backbone)):
                # pdb.set_trace()
                x_ = self.backbone[i](img)
                if self.with_neck:
                    x_=self.neck[i](x_)
                x.append(x_)
            x=tuple([x[0][0],x[0][1],x[1][0],x[1][1]])
        else:
            x = self.backbone(img)
            if self.with_neck:
                # multi-backbone
                if isinstance(self.neck, nn.ModuleList):
                    ...
                    for i in range(len(self.neck)):
                        x_ = self.neck[i](x[i])
                        x_backbone.append(x_[0])
                        x_backbone.append(x_[1])
                        # x_backbone.append(x_[2])
                        # x_backbone.append(x_[3])
                        # x_backbone.append(x_[4])
                        if i==1:
                            # x_backbone.append(x_[1])
                            x_backbone.append(x_[2])
                    x=tuple(x_backbone)
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            bbox_feats = self.bbox_roi_extractor(
                x[:self.bbox_roi_extractor.num_inputs], rois)  # todo: 1.3x roi  , roi_scale_factor=1.3
